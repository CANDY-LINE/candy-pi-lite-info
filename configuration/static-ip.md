...
**RASPBIAN STRETCHでは動作しません。更新までお待ちください**
...
そのような手間を軽減するため、CANDY Pi Lite向けのセットアップしたラズパイでは、SDカードに固定IPの設定をお使いにパソコンから保存することによって固定IPを設定できる方法を提供しています。

**ご注意** この方法は、すべてのラズパイに適用できる方法ではありません。CANDY Pi Lite向けのセットアップをされていない通常のラズパイについては、[固定IPをラズパイに設定](https://www.google.co.jp/#q=固定IPをラズパイに設定&tbs=qdr:y)を参照して実現してください。

## 全体の手順

CANDY Pi Lite向けのセットアップしたラズパイに、固定IPを設定するには、以下の手順を実施します。

...
### 接続先ネットワーク種類（3G/4G LTE）の限定

**👉 この設定は、上級者向けです。この内容があてはまるか不明な場合は、この設定を行なってはいけません。**

CANDY Pi Lite+をご利用の場合、回線契約やご利用中のキャリアが3G/4G LTE共に利用できる時は、CANDY Pi Liteは自動的に使用する接続先ネットワーク種類を決定します。
ご利用者の方にて任意の接続先ネットワーク種類を使用したい時は、以下のようにAPN設定にて`"nw"`を指定します。
